...
const { data, fetchMore, loading, isLastPage } = await usePaginatedFetch<Artist>(DEEZER().artists, { limit: 40 })
...
<template>
	<MusicContainer lt-md:px2 :always-grid="true" grid-class="!grid-cols-minmax-10rem lg:!grid-cols-minmax-10rem"
		title="Explore Artists">
		<CardArtist v-for="(artist, j) in data" :key="j" :artist="artist" />
	</MusicContainer>
