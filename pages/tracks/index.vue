...
const { data, fetchMore, loading, isLastPage } = await usePaginatedFetch<Track>(`${DEEZER().tracks}`, { limit: 30 })
...
<template>
	<MusicContainer lt-md:px2 :always-grid="true" grid-class="!grid-cols-minmax-12rem lg:!grid-cols-minmax-15rem"
		...
		<CardTrack v-for="(track, j) in data" :key="j" data-aos="fade-left" :data-aos-delay="100 * j"
			:data-aos-offset="-300" :track="track" />
	</MusicContainer>
