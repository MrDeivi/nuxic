...
<template>
  <div v-if="data" p="x2 y3" lt-md:py-0 lt-md:px-3 overflow-hidden>
    <div absolute top-0 left-0 z-1 blur-2xl w-full h80 op20 lt-md:op40 overflow-hidden>
      <NuxtImg
        v-if="data.album?.cover_small"
        animate-fade-in animate-duration-400
        :src="data.album?.cover_small" object-cover wfull hfull
      />
    </div>
      ...
      <div flex flex-row lt-md:flex-col>
        <div flex flex-row lt-lg:mb5>
          <div
            ...
            overflow-hidden w-50 h-50 min-w-50 min-h-50 relative
            class="lt-md:(h-40 w-40 min-h-40 min-w-40)"
          >
            <ImageBlurLoader
              v-if="data.album?.cover_medium"
              :big-image-src="data.album?.cover_medium"
              :small-image-src="data.album?.cover_small"
              class="object-cover wfull hfull"
            />

            <div
              v-if="!isCurrent" bg-white:20 absoluteCenter
              rounded-full p2 absolute transform cursor-pointer
              @click="play"
            >
          ...
          >
            ...
            <h2 lt-md:text-sm>
              <span min-w20 lt-md:min-w17 inline-flex>Album:</span> <NuxtLink :to="`/albums/${data.album?.id}`" class="link">
                {{ data.album?.title }}
              </NuxtLink>
            </h2>
            <h2 lt-md:text-sm>
